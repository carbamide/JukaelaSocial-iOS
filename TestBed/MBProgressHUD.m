...
-(id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.opacity = 0.8f;
        self.labelFont = [UIFont fontWithName:kFontPreference size:LABELFONTSIZE];
        self.detailsLabelFont = [UIFont fontWithName:kFontPreference size:LABELDETAILSFONTSIZE];
        self.xOffset = 0.0f;
        self.yOffset = 0.0f;
...
-(void)layoutSubviews {
	...
	if (square) {
		CGFloat max = jMAX(self.width, self.height);
...
-(void)drawRect:(CGRect)rect {
    ...
    if (dimBackground) {
        ...
        //Gradient radius
        float gradRadius = jMIN(self.bounds.size.width , self.bounds.size.height) ;
        //Gradient draw
